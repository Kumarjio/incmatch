...
<?php
		...
		$ses = "yes";
			...
			if(isset($_SESSION['userName'])){
				if(((time() - $_SESSION['last_time']) > 300)){
					...
					echo "<script>window.location.assingn = '" . LOGOUT . "';</script>";
					//exit;
					$ses = "no";
				}
				else{
	...
	function sessionTimeOutUser(){
		...
		if(isset($_SESSION['cwwusername'])){
			if(((time() - $_SESSION['user_time']) > 300)){
				//$session_unset();
				echo "<script>alert('You are being logged out because of inactivity.'); </script>";
...
function getApplicant($name, $email, $id, $empId, $jobTitle){
	...
	function getEverythingAfterSignInUrl($sign, $stop){
		$count = 0;
		$actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
		...
		$clickedResume = substr($actual_link, $count + 1);
		$count2 = 0;
		while((strcmp($clickedResume[$count2], "$stop"))){
			
			$count2++;
		}
		$clickedRes = substr($clickedResume, 0, $count2);
		return $clickedRes;
		//return $clickedResume;
	}	

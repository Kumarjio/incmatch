...
ob_start();
	...
	define("EMAIL", "kraymond@whatcom.edu"); //admin emali that will receive all the communication emails
	define("HEADER", "FROM: kraymond@whatcom.edu"); //header to use with mailto();
	define("HOME", "http://cwwinternship.cyberwatchwest.org/internship/index.php"); //home constant
	define("EMPLOGIN", "http://cwwinternship.cyberwatchwest.org/internship/mylogin.php"); //Employer login page constant
	define("USERLOGIN", "http://cwwinternship.cyberwatchwest.org/internship/cwwsignin.php"); //User login page constant
	define("EMPSIGNUP", "http://cwwinternship.cyberwatchwest.org/internship/employers.php"); //Employer signup page constant
	define("VERIFIED", "http://cwwinternship.cyberwatchwest.org/internship/verified.php");	 //Verified page constant
	define("GOVERIFY", "http://cwwinternship.cyberwatchwest.org/internship/goverify.php");		
	define("ACCESS", "http://cwwinternship.cyberwatchwest.org/internship/accesscodegenerator.php");	
	define("ACODE", "http://cwwinternship.cyberwatchwest.org/internship/adcode.php");	
	define("ASIGNIN", "http://cwwinternship.cyberwatchwest.org/internship/adminsignin.php");
	define("UNAUTHMSG", "http://cwwinternship.cyberwatchwest.org/internship/unauthmessage.php");	
	define("APPLICANTS", "http://cwwinternship.cyberwatchwest.org/internship/applicants.php");	
	define("SCHOOLCHECK", "http://cwwinternship.cyberwatchwest.org/internship/schoolcheck.php");
	define("ADSIGNUP", "http://cwwinternship.cyberwatchwest.org/internship/adsignup.php");
	define("EMPHOME", "http://cwwinternship.cyberwatchwest.org/internship/mypage.php");
	define("SKRHOME", "http://cwwinternship.cyberwatchwest.org/internship/home.php");
	define("SKRSIGNUP", "http://cwwinternship.cyberwatchwest.org/internship/seekersignup.php");
	define("SKRSIGNIN", "http://cwwinternship.cyberwatchwest.org/internship/cwwsignin.php");
	define("PASSRESET", "http://cwwinternship.cyberwatchwest.org/internship/passreset.php");
	define("PASSRESETTEMP", "http://cwwinternship.cyberwatchwest.org/internship/passresettemp.php");
	define("SKRSEARCH", "http://cwwinternship.cyberwatchwest.org/internship/cwwemployersearch.php");		
...
*/
	...
	<?php
			while($result4 = mysqli_fetch_array($query2)){
				...
				?>								
				<?php				
				echo "<div class='' style='border-bottom: 8px solid #000000; margin-bottom: 2%;  white-space: pre-wrap; white-space: -moz-pre-wrap;  white-space: -pre-wrap;  white-space: -o-pre-wrap;  word-wrap: break-word '><strong>" . strToUpper($result4['company']) . "</strong><br><br>";
				echo  "<strong>INTERNSHIP TYPE: </strong><br>" . $result4['job_type'] . "<br><br>";
				echo  "<strong>INTERNSHIP TITLE: </strong><br>". $result4['job_title'] . "<br><br>";
				echo  "<strong>INTERNSHIP DESCRIPTION: </strong><br>" . $result4['description'] . "<br><br>";
				echo  "<strong>INTERNSHIP QUALIFICATIONS: </strong><br>" . $result4['qualifications'] . "<br><br>";
				echo "<strong>INTERNSHIP LENGTH: </strong><br>" . $result4['internship_length'] . "<br><br>";
				echo  "<strong>INTERNSHIP LOCATION: </strong><br>" . $result4['zip'] . "<br>";
				echo "<a style='color: #000000' href=" . $result4['link'] . ">" . $result4['link'] . "</a><br>";
				?>				
				</div>
				<?php
				}
				else{
					echo "we are on our way";
				}
			}
	...
	function getEmployers(){
		...
		while($result = mysqli_fetch_array($query)){
			...
			if($imgs){
				...
				echo "<div class='jobInfo' style='display: inline-block'><a   style='color: #000000' href=" . "joblistings.php?company=" . $compAbbrev[$count] . "><img src='data:image/png;base64," . base64_encode( $result['Image'] ) . "' style='padding: 1em' alt='cyberwatch west logo' /><br>" . $info . "</a></div>";
				//echo $compName[$count];
				$count++;
				$id = $result['id'];
					...
					if($result['Password'] == $secure){
						...
						while($results2 = mysqli_fetch_array($queryRegistered)){
							if($_SESSION['cwwid'] == $results2['user_id'] && $results2['group_id'] == 9){
								...
								$_SESSION['cwwhomepage'] = "http://cwwinternship.cyberwatchwest.org/internship/home.php?id=" . $_SESSION['cwwid'];
...
<?php
	...
	function sessionTimeOut(){
		...
		if($ses){
			...
			//echo "<script> window.location = 'http://cwwinternship.cyberwatchwest.org/internship/logout.php';</script>";
		}
...
function jobPref($job, $zip){
		...
		if($query){
			...
			if($check){
				...
				$URL="http://cwwinternship.cyberwatchwest.org/internship/home.php?id=" . $_SESSION['cwwid'];
				echo "<script type='text/javascript'>document.location.href='" . $URL . "'</script>";
				echo '<META HTTP-EQUIV="refresh" content="0;URL=' . $URL . '">';
	...
	function applicantMatch(){
		...
		if($queryPref){
			...
			while($result = mysqli_fetch_array($queryPref)){
				...
				$sqlIntern = "SELECT * FROM `internships` ORDER BY `id` DESC";
						...
						if($close > $myResults['created_at']){
							...
							if(strcmp($myResults['job_type'], $job) == 0){
								...
								//internshipCheck($myResults['employer_id'], $myResults['y_day']);
									...
									echo "<div class='' style='border-radius: 10px; padding: 2%; color: #ffffff; border-bottom: 8px solid #000000'>Posted on  <span style='color: #ff0'> " . $myResults['created_at'] . "  </span> By<span style='color: #febf00'><strong>  " . strToUpper($myResults['company']) . "</strong></span><br><br>";
									...
									if($myResults['internship_length'] == 1){
									echo "<strong>Internship Length: </strong>" . $myResults['internship_length'] . "Month<br><br>";
									}
									elseif($myResults['internship_length'] > 1){
										echo "<strong>Internship Length: </strong>" . $myResults['internship_length'] . "Months<br><br>";
									}
									else{
										echo "Internship Length not available";
									}
									...
									echo $myResults['created_at'] . "<br>";
									echo "<strong>Opportunity Closes:  " . $daysLeft . "</strong></div>";
									?>
	...
	if($query){
		...
		while($results = mysqli_fetch_array($query)){			
			...
			if($daysLeft == 5){
				...
				$mail = mail($_SESSION['email'], "Your CWW listing is expiring soon!", "Please give " .$username . " " . $email . " an access code http://cwwinternship.cyberwatchwest.org/internship/accesscodegenerator.php", "From: " . $nResults['Email']);
			}
...
function verify($to, $from, $id){
		...
		if($queryUpdate){
			...
			if($query2){
				...
				$conn = dbConnectNow();				
				$queryInsert = mysqli_query($conn, "INSERT INTO `employers`(`id`, `Username`, `Email`, `Address`, `Password`, `Image`, `companyName`, `abbrev`, `member_group`) SELECT * FROM `temp_employers` WHERE `id`='$id'");
				if($queryInsert){
...
function displayApplicants($jobTitle){
	...
	if($query){
		while($results = mysqli_fetch_array($query)){
			$applicantArray[$count] = array("name"=>$results['Name'], "email"=>$results['Email'], "jobType"=>$results['job_type'], "userId"=>$results['user_id']);
			$count++;
		}
